...
\begin{equation}
  ...
  r = \frac{2}{\left(\kappa - 1\right) \text{Ma_e}^2} \left(\frac{T_\text{a_{w}}}{T_\text{e}} - 1\right) \approx
  \begin{cases}
...
Die Bossinesq-Approximation kann zu Modellierung von Auftrieb infolge von geringen Dichteänderungen verwendet werden. Die Dichte $\rho$ wird
somit als konstant angenommen, bis auf den Auftriebsterm der Impulsgleichung. Dort gilt:

\begin{equation}
  \label{eq:bossinesque}
  \rho = \rho_0 \left[1-\gamma \left(T-T_0\right)\right]
\end{equation}

Hierbei ist $\rho_0$ die Referenzdichte, $\gamma$ der Wärmeausdehnungskoeffizient und $T_0$ die Referenztemperatur.
Diese Approximation kann den Rechenaufwand erheblich verringern und ist für folgende Bedingungen gültig:
...
Die Dichte kann in Abhängigkeit des Flüssigkeitsanteils $\Gamma$ berechnet werden. Zwischen der Dichte am Liquidus- und Soliduspunkt
wird linear interpoliert:

\begin{equation}
  \label{eq:pcm_dichte}
  \rho(\Gamma) = \left(1- \Gamma\right) \rho_\text{solidus} + \Gamma \rho_\text{liquidus} \qquad \Gamma \in [0,1]
\end{equation}