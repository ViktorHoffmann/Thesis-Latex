...
meist bewegliche Bauteile wie Pumpen, welche die Ausfallwahrscheinlichkeit erhöhen. Alternativ gibt auch
...
\subsection{Wärmerohre}\label{sec:waermerohre}

Wärmerohre sind eine Möglichkeit durch erzwungene Konvektion Wärme zu transportieren. Sie sind vollständig geschlossene Rohre mit einer
Flüssigkeit im inneren und einer Kapillarstruktur an der Innenwand, so dass ein freier Kanal in der Mitte bleibt. Bei der Wärmequelle
...
Kreislauf bildet. Besonders effektiv sind Wärmerohre durch die Nutzung der Verdampfungsenthalpie beim Flüssig-Gas Übergang an der Wärmequelle,
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \draw[->, thick, -{Stealth[length=0.25cm]}] (1,-2.75) -- node [midway, above] {Flüssigkeitsstrom}(-1,-2.75); % Flüssigkeitspfeil
...
Ein Nachteil von Wärmerohren ist, dass der Wärmeleitwiederstand durch Biegungen und Anbindung von mehreren Quellen um bis zu \SI{100}{\percent}
steigen kann \cite{Mooney-2020}.
...
Wegen der hohe Wärmeleitzahl von \ac{pgs} und besonders für Thermal Straps wichtigen Flexibilität, sind diese eine interessante Option.
...
\begin{figure}[H]
  ...
  \caption{Kommerziell erhältliche Thermal Straps aus Graphen, Kupfer und Aluminium~\cite{Thermal-Straps}}\label{fig:thermalstraps_commercial}
\end{figure}
...
\begin{table}[H]
  \centering
  \caption{Vergleich von Materialien für Wärmeleitbänder}\label{tab:strap_materials}
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}m{3cm} m{3.2cm} m{3.2cm} m{3cm}}
    \toprule[1pt]
    \textbf{Eigenschaft} & \textbf{Kupfer}\cite{Thermtest-DB} & \textbf{Aluminium}\cite{Thermtest-DB} & \textbf{PGS \nobreak{(Graphit)}}\cite{HPMS-PGS} \\
    \toprule[1pt]
